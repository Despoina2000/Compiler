/* This file was generated by SableCC (http://www.sablecc.org/). */

package minipython.node;

import java.util.*;
import minipython.analysis.*;

public final class APercentMultiplication extends PMultiplication
{
    private PMultiplication _multiplication_;
    private TPercent _percent_;
    private PPower _power_;

    public APercentMultiplication()
    {
    }

    public APercentMultiplication(
        PMultiplication _multiplication_,
        TPercent _percent_,
        PPower _power_)
    {
        setMultiplication(_multiplication_);

        setPercent(_percent_);

        setPower(_power_);

    }
    public Object clone()
    {
        return new APercentMultiplication(
            (PMultiplication) cloneNode(_multiplication_),
            (TPercent) cloneNode(_percent_),
            (PPower) cloneNode(_power_));
    }

    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAPercentMultiplication(this);
    }

    public PMultiplication getMultiplication()
    {
        return _multiplication_;
    }

    public void setMultiplication(PMultiplication node)
    {
        if(_multiplication_ != null)
        {
            _multiplication_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        _multiplication_ = node;
    }

    public TPercent getPercent()
    {
        return _percent_;
    }

    public void setPercent(TPercent node)
    {
        if(_percent_ != null)
        {
            _percent_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        _percent_ = node;
    }

    public PPower getPower()
    {
        return _power_;
    }

    public void setPower(PPower node)
    {
        if(_power_ != null)
        {
            _power_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        _power_ = node;
    }

    public String toString()
    {
        return ""
            + toString(_multiplication_)
            + toString(_percent_)
            + toString(_power_);
    }

    void removeChild(Node child)
    {
        if(_multiplication_ == child)
        {
            _multiplication_ = null;
            return;
        }

        if(_percent_ == child)
        {
            _percent_ = null;
            return;
        }

        if(_power_ == child)
        {
            _power_ = null;
            return;
        }

    }

    void replaceChild(Node oldChild, Node newChild)
    {
        if(_multiplication_ == oldChild)
        {
            setMultiplication((PMultiplication) newChild);
            return;
        }

        if(_percent_ == oldChild)
        {
            setPercent((TPercent) newChild);
            return;
        }

        if(_power_ == oldChild)
        {
            setPower((PPower) newChild);
            return;
        }

    }
}
